# ============================================================
# DSAR-RL Phase 9 - ICD Scanning (Semgrep + Regex) for Colab
# ============================================================
# INSTRUCTIONS:
# 1. Upload this script to Google Colab
# 2. Upload all 5 test output JSON files to Google Drive folder: MyDrive/dsar_phase9/
#    - baseline_test_outputs.json
#    - zeroshot_test_outputs.json
#    - thea_static_test_outputs.json
#    - semantic_static_test_outputs.json
#    - bstar_test_outputs.json
# 3. In Colab, run:
#    !pip install semgrep
#    from google.colab import drive; drive.mount('/content/drive')
#    !python colab_scan_phase9.py
# 4. Download the *_icd.json output files from Drive folder back to Windows
#    and place in: outputs/phase9/
# ============================================================
...
# --- SET THIS TO YOUR DRIVE FOLDER ---
DRIVE_DIR = Path("/content/drive/MyDrive/dsar_phase9")
...
def scan_file(out_path, icd_path):
    ...
    if not out_path.exists():
        print(f"SKIP (not found): {out_path}")
        return

    if icd_path.exists():
        print(f"SKIP (already done): {icd_path.name}")
        return
...
def main():
    ...
    print(f"Files found: {os.listdir(DRIVE_DIR)}\n")

    scan_pairs = [
        ("baseline_test_outputs.json",         "baseline_test_icd.json"),
        ("zeroshot_test_outputs.json",          "zeroshot_test_icd.json"),
        ("thea_static_test_outputs.json",       "thea_static_test_icd.json"),
        ("semantic_static_test_outputs.json",   "semantic_static_test_icd.json"),
        ("bstar_test_outputs.json",             "bstar_test_icd.json"),
    ]

    for out_name, icd_name in scan_pairs:
        print(f"\n=== Scanning {out_name} ===")
        scan_file(DRIVE_DIR / out_name, DRIVE_DIR / icd_name)

    print("\nAll scans complete.")
    print("Download the *_icd.json files from your Drive folder.")
